 ...
 * ソフトウェアの構成要素(クラス・モジュール・関数など)は拡張に対して開いて(オープン: Open)いて
 * 修正に対して閉じて(クローズド: Closed)いなければならない。
 *
 ...
 *
 * オープン・クローズドの原則の鍵は、「抽象」にあります。モジュールをある固定した「抽象」に依存させておけば、
 * 修正に対してコードを閉じることができるようになります。「抽象」を使えば、
 * 派生クラスを新たに追加するだけで、振る舞いを拡張できます。
 *
 */
package solid;
